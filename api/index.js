...
${partnerName} and you should now discuss this problem and reach a definitive final answer in the format: ${problem.expectedFormat}.`
    ...
    if (provider === "openai" && apiKeys?.openai) {
      ...
      const ai = apiKeys?.google ? new GoogleGenAI({
...
app.post("/api/huggingface/publish", async (req, res) => {
  try {
    ...
    const commitOps = auditedOperations.map((op) => ({
      key: "file",
      value: {
        path: op.path,
    ...
    const commitPayload = {
      summary: req.body.summary && typeof req.body.summary === "string" && req.body.summary.trim() || `Upload 100% Verified DualBlind Arena Reasoning Dataset (${verifiedCount > 0 ? verifiedCount + " records" : "metadata"})`,
