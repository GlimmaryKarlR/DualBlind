...
// Single model probe diagnostic endpoint
app.post('/api/models/probe', async (req, res) => {
  const startTime = Date.now();
  try {
    const { model, provider, apiKeys, customEndpoint } = req.body;
    if (!model) {
      return res.status(400).json({ ok: false, error: 'Missing model name to probe' });
    }

    const resolvedId = resolveOpenRouterModel(model);
    const selectedProvider = (provider || 'openrouter').toLowerCase();

    const openRouterKey = apiKeys?.openrouter || process.env.OPENROUTER_API_KEY;
    const orcaRouterKey = apiKeys?.orcarouter || process.env.ORCAROUTER_API_KEY;
    const geminiKey = apiKeys?.google || process.env.GEMINI_API_KEY;

    let endpoint = 'https://openrouter.ai/api/v1/chat/completions';
    let apiKey = openRouterKey;

    if (selectedProvider === 'orcarouter' || (orcaRouterKey && !openRouterKey)) {
      endpoint = customEndpoint || apiKeys?.orcarouterEndpoint || process.env.ORCAROUTER_BASE_URL || 'https://api.orcarouter.com/v1/chat/completions';
      apiKey = orcaRouterKey || openRouterKey;
    } else if (customEndpoint || apiKeys?.customEndpoint?.baseUrl) {
      endpoint = (customEndpoint || apiKeys?.customEndpoint?.baseUrl).replace(/\/$/, '') + '/chat/completions';
      apiKey = apiKeys?.customEndpoint?.apiKey || apiKey;
    }

    if (selectedProvider === 'google' && geminiKey) {
      const lower = String(model).toLowerCase();
      let googleModel = 'gemini-3.6-flash';
      if (lower.includes('3.7')) googleModel = 'gemini-3.7-flash';
      else if (lower.includes('pro')) googleModel = 'gemini-3.1-pro-preview';
      else if (lower.includes('3.6')) googleModel = 'gemini-3.6-flash';
      else if (lower.includes('3.5')) googleModel = 'gemini-3.5-flash';
      else if (lower.includes('flash')) googleModel = 'gemini-3.6-flash';

      const ai = new GoogleGenAI({ apiKey: geminiKey, httpOptions: { headers: { 'User-Agent': 'aistudio-build' } } });
      const geminiRes = await ai.models.generateContent({
        model: googleModel,
        contents: [{ parts: [{ text: 'Ping. Reply PONG.' }] }],
      ...
      return res.json({
        ok: true,
        model,
        resolvedId: googleModel,
        provider: 'google',
        latencyMs,
        response: geminiRes.text?.slice(0, 100),
      });
    }
    ...
    if (!apiKey) {
      return res.status(400).json({
        ok: false,
        error: 'No API key provided or found in server environment for this provider.',
        suggestedFix: 'Configure your OpenRouter, OrcaRouter, or Gemini key in the APIs & Tokens locker.',
      });
    }

    const probeResult = await callOpenAICompatible(
      endpoint,
      apiKey,
      resolvedId,
      [
    ...
    return res.json({
      ...
      model,
      resolvedId,
      actualModelUsed: probeResult.modelUsed,
      latencyMs,
      response: probeResult.text.slice(0, 100),
      usage: probeResult.usageMetadata,
    });
  } catch (err: any) {
    const latencyMs = Date.now() - startTime;
    const msg = err.message || String(err);
    let errorCategory = 'INFERENCE_ERROR';
    let suggestedFix = 'Verify the model ID exists on your selected router or provider.';

    if (msg.includes('404') || msg.toLowerCase().includes('not found')) {
      errorCategory = 'MODEL_NOT_FOUND_OR_DEPRECATED';
      suggestedFix = 'The upstream provider does not host this model ID. Check if it was renamed or replaced.';
    } else if (msg.includes('401') || msg.includes('403') || msg.toLowerCase().includes('auth')) {
      errorCategory = 'AUTH_OR_PERMISSION_DENIED';
      suggestedFix = 'Verify your API key is valid and has sufficient credits or accepted model terms.';
    } else if (msg.includes('429')) {
      errorCategory = 'RATE_LIMITED';
      suggestedFix = 'Rate limit or quota exceeded. Wait a few moments or upgrade provider tier.';
    } else if (msg.toLowerCase().includes('system') || msg.toLowerCase().includes('temperature')) {
      errorCategory = 'PARAMETER_REJECTED';
      suggestedFix = 'Model does not accept system messages or requires default temperature.';
    }
